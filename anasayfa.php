...
<ul>
<?php
	$resim	=	mysqli_query($baglanti,"SELECT * FROM slide ORDER BY id ASC");
	while($resimler=mysqli_fetch_array($resim))
	{
...
<div class="vitrin">
	<?php
        $vitrinsql	=	mysqli_query($baglanti,"SELECT urun.urun_id,urun.urun_adi,urun.urun_resmi,deger.fiyat FROM urun,deger WHERE urun.vitrin=1 AND urun.urun_id=deger.urun_id");
...
<div class="vitrin">
	<?php
		@$tarih=date("Y-m-d");
        $indirimsql	=	mysqli_query($baglanti,"SELECT urun.urun_id,urun.urun_adi,urun.urun_resmi,deger.indirim_orani FROM 
        urun,deger 
        WHERE deger.indirim_orani!=0
         AND deger.bas_tarih<='$tarih' 
         AND bit_tarih>='$tarih' AND
          deger.urun_id=urun.urun_id 
          ORDER BY deger.indirim_orani DESC");

        while($indirimkayitlar=@mysqli_fetch_array($indirimsql))
        {
			
			$fiyatsql=@mysqli_query($baglanti,"SELECT * from deger WHERE urun_id=$indirimkayitlar[0]");
